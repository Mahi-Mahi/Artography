		...
		function($scope, $rootScope, $location, $route, dataService, langService, formatService) {
			...
			function updateStatus() {
				...
				if (dataService.data.today.events.length) {
					...
					if (the_event.a) {
						text = '<li class="news-block-item"><a href="/artiste/' + the_event.a + '/"><strong><span>' + the_event.an + '</span>' + the_event.n + '</strong> @' + the_event.ct + ' -  ' + the_event.c + ' / ' + (the_event.d ? (formatService.formatLongDate(the_event.d[0]) + (the_event.d[1] ? (' - ' + formatService.formatLongDate(the_event.d[1])) : '')) : '') + ' </a></li>';
