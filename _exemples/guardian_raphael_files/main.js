...
function resizeChart(divW, divH, orderKey, dataTable, chartData, type) {
	...
	console.log([divW, divH, orderKey, dataTable, chartData, type]);
		...
		for (var l = 0; l < orderKey.length; l++) {
			...
			for (var i = 0; i < dataTable.length; i++) {
				...
				if (displayType != 'friends' && prevDisplayType != 'friends') {
					...
					if ($.browser.msie) {
						chartData[i]['layersObj'][orderKey[l]].attr({
							'path': path,
							'opacity': 1
						});
					} else {
						chartData[i]['layersObj'][orderKey[l]].animate({
							'path': path,
							'opacity': 1
						}, 150, "easeIn");
					}
				...
				if (l == 0) {
					...
					chartData[i]['label'].attr({
						'x': originX + Math.sin(labelA) * (fullRadius + 10),
						'y': originY + Math.cos(labelA) * (fullRadius + 10)
					})
					...
					if (chartData[i]['regionDivider'] != false) {
						...
						if (displayType != 'friends') {
							...
							chartData[i].regionDivider.attr({
								'path': line,
								'stroke-opacity': 1
							})
							chartData[i].regionLabel = charts[type].text(originX + Math.sin(regionLabelA) * (fullRadius + 30), originY + Math.cos(regionLabelA) * (fullRadius + 30), chartData[i].region.toUpperCase()).attr({
								'text-anchor': 'middle',
								"font-size": 10,
								fill: "#333",
								'font-weight': 'bold',
...
function drawCircleChart(div, canvasH, canvasW, divW, divH, offsetX, offsetY, orderKey, dataTable, type) {
	...
	dataTable = dataTable.slice(0, 3);
	orderKey = orderKey.slice(0, 2);

	console.log("drawCircleChart");
	console.log([div, canvasH, canvasW, divW, divH, offsetX, offsetY, orderKey, dataTable, type]);
		...
		for (var i = 0; i < dataTable.length; i++) {
			...
			if (l == 0) {
				...
				if (dataTable[i]['state'].toLowerCase() == linkedState && type == 'overview') {
					...
					chartData[i]['hover'] = chart.path(outline).attr({
						"stroke-width": 2,
						"stroke": "#000000",
						'stroke-opacity': 1,
						'fill-opacity': 0,
						'fill': '#000'
					}).data('chartType', type).data('statePos', i).data('region', dataTable[i]['region'])
				...
				chartData[i]['label'] = chart.text(originX + Math.sin(labelA) * (fullRadius + 10), originY + Math.cos(labelA) * (fullRadius + 10), dataTable[i]['stateabbreviation']).attr({
					'text-anchor': 'middle',
					"font-size": 10,
					fill: "#ccc",
					"font-family": "Arial, Helvetica Neue, Helvetica, sans-serif",
				...
				if (curRegion == '') {
					...
					chartData[i].regionDivider = chart.path(line).attr({
						"stroke-width": 4,
						"stroke": "#000"
					})
					...
					var regionlabel = chart.text(originX + Math.sin(regionLabelA) * (fullRadius + 30), originY + Math.cos(regionLabelA) * (fullRadius + 30), curRegion.toUpperCase()).attr({
						'text-anchor': 'middle',
						"font-size": 10,
						fill: "#333",
						'font-weight': 'bold',
			...
			var block = chart.path(path).attr({
				fill: blockColor,
				"stroke-width": 1,
				"stroke": "#fff"
			}).toBack();
...
function getFriendLocations(type) {
	FB.api('/', 'POST', {
		batch: [{
			...
			'method': 'GET',
			'relative_url': '?ids={result=get-friends:$.data.*.id}&fields=location'
		}]
	}, function(response) {
...
$.receiveMessage(function(e) {
	...
	if (param[0] == 'state') {
		...
		$.postMessage({
			section_link: $('#' + param[1].toLowerCase() + '-section').offset().top
		}, parent_url, parent);
